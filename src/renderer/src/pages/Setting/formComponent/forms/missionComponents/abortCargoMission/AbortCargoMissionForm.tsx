...
const AbortCargoMissionForm: FC = () => {
  ...
  const setMissionMutation = useMutation({
    ...
    onSuccess: async () => {
      queryClient.refetchQueries({ queryKey: ['abort-mission-when-has-cargo'] }),
  ...
  const submit = () => {
    ...
    if (!payload.amrId || payload.amrId.length === 0) {
      messageApi.warning(`amrId${t('mission.abort_mission_when_has_cargo_mission.amr_warn')}`);
      return;
    }

    for (const key of Object.keys(payload) as Array<keyof SubmitPayload>) {
      if (payload[key] === null) {
        messageApi.warning(`${key}${t('mission.abort_mission_when_has_cargo_mission.missed')}`);
        return;
      }
    }
