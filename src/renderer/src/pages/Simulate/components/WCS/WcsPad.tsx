...
const WcsPad: FC = () => {
  ...
  const editMutation = useMutation({
    ...
    onSuccess: async () => {
      await Promise.all([queryClient.refetchQueries({ queryKey: ['all-mock-conveyor-config'] })]);
  ...
  return (
    <>
      ...
      {isOpen ? <WcsModal isOpen={isOpen} handleCancel={handleCancel} handleOk={handleOk} /> : []}
    </>
  );
};
